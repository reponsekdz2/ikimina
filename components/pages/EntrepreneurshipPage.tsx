...
const IdeaCard: React.FC<{ title: string; category: string }> = ({ title, category }) => (
    <div className="p-5 rounded-2xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-md border border-gray-200 dark:border-gray-700 group transform hover:-translate-y-2 transition-transform duration-300">
        <div className="flex items-center space-x-4">
            <div className="p-3 bg-yellow-100 dark:bg-yellow-900/50 rounded-full text-yellow-600 dark:text-yellow-300">
                <LightbulbIcon className="w-6 h-6"/>
            </div>
            <div>
                <h3 className="font-bold text-gray-900 dark:text-white">{title}</h3>
                <p className="text-sm text-yellow-700 dark:text-yellow-400 font-semibold">{category}</p>
            </div>
        </div>
        <a href="#" className="text-sm font-semibold text-brand-blue dark:text-brand-yellow mt-4 inline-block opacity-0 group-hover:opacity-100 transition-opacity">Learn more &rarr;</a>
    </div>
);
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                 ...
                 <div className="space-y-6">
                    <div className="p-6 rounded-2xl bg-gradient-to-br from-brand-blue to-brand-green text-white shadow-lg">
                        ...
                        <RippleButton className="w-full mt-4 bg-white/20 hover:bg-white/30 text-white hover:shadow-lg">Start Pitch</RippleButton>
                    </div>
                </div>
            </div>
        </div>
