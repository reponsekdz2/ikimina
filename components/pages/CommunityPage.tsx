...
const PostCard: React.FC<{post: CommunityPost}> = ({post}) => (
    <div className="p-5 rounded-2xl bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 transform hover:scale-[1.02] transition-transform duration-300">
        ...
        <div className="flex space-x-4 mt-4 text-sm text-gray-500 dark:text-gray-400">
            <button className="hover:text-[#1E90FF]">Like ({post.likes})</button>
            <button className="hover:text-[#32CD32]">Comment ({post.comments})</button>
        </div>
    </div>
)
        ...
        <div className="p-4 bg-white/50 dark:bg-gray-800/50 rounded-2xl">
            ...
            <div className="grid grid-cols-7 text-center mt-2">
                ...
                {Array.from({length: daysInMonth}).map((_, i) => (
                    <div key={i+1} className={`p-1.5 rounded-full cursor-pointer ${i+1 === date.getDate() ? 'bg-[#1E90FF] text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}>{i+1}</div>
                ))}
            </div>
        </div>
    )
        ...
        <div className="space-y-12">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="p-5 rounded-2xl bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700">
                        <textarea className="w-full p-2 bg-transparent focus:outline-none placeholder:text-gray-500" placeholder="Share your success story..."></textarea>
                        <button className="mt-2 w-full py-2.5 font-semibold text-white bg-gradient-to-r from-[#1E90FF] to-[#20B2AA] hover:shadow-lg hover:shadow-teal-500/50 rounded-full transition-shadow">Post</button>
                    </div>
                ...
                <div className="space-y-6">
                    <div className="p-4 rounded-2xl bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700">
                        ...
                        <ul className="mt-4 text-sm space-y-2">
                            <li className="flex items-center"><span className="w-2 h-2 rounded-full bg-[#32CD32] mr-3"></span> Webinar: Financial Literacy</li>
                            <li className="flex items-center"><span className="w-2 h-2 rounded-full bg-[#1E90FF] mr-3"></span> Kigali Tech Meetup</li>
                        </ul>
                    </div>
                     <div className="p-4 rounded-2xl bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700">
                        <h3 className="font-bold font-display mb-2">Forum Topics</h3>
                        <ul className="space-y-1 text-sm text-[#20B2AA] dark:text-teal-400">
