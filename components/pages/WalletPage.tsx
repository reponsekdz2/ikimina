    ...
    return (
        <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
            <svg className="w-full h-full transform -rotate-90">
                ...
                <circle className="text-brand-green transition-all duration-500 ease-out" strokeWidth={strokeWidth} strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" stroke="currentColor" fill="transparent" r={radius} cx={size/2} cy={size/2} />
            </svg>
            <span className="absolute text-xl font-bold font-display text-brand-green">{Math.round(percentage)}%</span>
        </div>
    );
};
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-[100] p-4 animate-fade-in-up">
            <div className="w-full max-w-lg bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-3xl shadow-2xl p-8 relative" onClick={e => e.stopPropagation()}>
                ...
                <h3 className="text-2xl font-bold font-display text-center mb-4">Apply for a Loan</h3>
                {step === 1 ? (
                    <div className="space-y-6">
                        ...
                        <RippleButton onClick={handleSubmit} className="w-full text-white bg-gradient-to-r from-brand-blue to-brand-green disabled:opacity-75" disabled={isSubmitting}>
...
const LoanRepaymentModal: React.FC<{ loan: { total: number, paid: number }; currentBalance: number; onClose: () => void; onRepay: (amount: number) => void; }> = ({ loan, currentBalance, onClose, onRepay }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-[100] p-4 animate-fade-in-up">
            <div className="w-full max-w-lg bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-3xl shadow-2xl p-8 relative" onClick={e => e.stopPropagation()}>
                 <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white transition-colors"><XIcon className="w-6 h-6"/></button>
                 <h3 className="text-2xl font-bold font-display text-center mb-4">Repay Loan</h3>
                 {step === 1 ? (
                    <div className="space-y-4">
                        ...
                        <div>
                             <div className="flex justify-between items-center">
                                ...
                                <button onClick={() => setAmount(remaining)} className="text-sm font-semibold text-brand-blue hover:underline">Pay Full Amount</button>
                            </div>
                            <input 
                                type="number" 
                                value={amount} 
                                ...
                                max={remaining}
                                min="1"
                                className={`w-full mt-1 p-2 rounded-md bg-gray-100 dark:bg-gray-700 border ${error ? 'border-red-500' : 'border-transparent'} focus:ring-2 focus:ring-brand-blue`}
                            />
                            {error && <p className="text-xs text-red-500 mt-1">{error}</p>}
                        </div>
                        <RippleButton 
                            onClick={handleRepay} 
                            className="w-full text-white bg-gradient-to-r from-brand-blue to-brand-green disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <div className="space-y-12">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
                <div className="p-8 rounded-2xl bg-gradient-to-br from-brand-blue to-brand-green text-white shadow-2xl">
                    ...
                    <div className="flex space-x-4 mt-8">
                        <RippleButton className="bg-white/20 hover:bg-white/30 text-white">Deposit</RippleButton>
                        <RippleButton onClick={() => activeLoan ? setIsRepayModalOpen(true) : setIsApplyModalOpen(true)} className="bg-white/20 hover:bg-white/30 text-white">
                            {activeLoan ? 'Repay Loan' : 'Apply for Loan'}
                        </RippleButton>
                    </div>
                </div>

                <div className="p-6 rounded-2xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-md border border-gray-200 dark:border-gray-700 relative">
                     <div className="absolute top-0 left-0 h-full w-1.5 bg-gradient-to-b from-brand-blue to-brand-green rounded-l-2xl"></div>
                     <h3 className="text-xl font-bold font-display text-gray-900 dark:text-white mb-4 ml-2">Recent Transactions</h3>
                ...
                <div className="p-6 rounded-2xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-md border border-gray-200 dark:border-gray-700 relative">
                    <div className="absolute top-0 left-0 h-full w-1.5 bg-gradient-to-b from-yellow-400 to-orange-500 rounded-l-2xl"></div>
                    <h3 className="text-xl font-bold font-display text-gray-900 dark:text-white mb-4 ml-2">Loan History</h3>
                    <ul className="space-y-3">
                 ...
                 <div className="p-6 rounded-2xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-md border border-gray-200 dark:border-gray-700">
                     ...
                     {activeLoan ? (
                        <div className="space-y-3 flex flex-col items-center">
                            ...
                            <RippleButton onClick={() => setIsRepayModalOpen(true)} className="w-full bg-brand-blue text-white">Make a Payment</RippleButton>
                        </div>
                     ) : (
                        <div className="text-center">
                            <p className="text-gray-600 dark:text-gray-400 mb-4">You have no active loans. Need funds for your next project?</p>
                            <RippleButton onClick={() => setIsApplyModalOpen(true)} className="w-full bg-gradient-to-r from-brand-blue to-brand-green text-white hover:shadow-lg hover:shadow-cyan-500/50">Apply Now</RippleButton>
                        </div>
                     )}
                 </div>
            </div>
